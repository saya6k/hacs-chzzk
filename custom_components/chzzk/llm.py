"""LLM tools platform for Chzzk.

Discovered lazily by Home Assistant's `llm` integration (HA 2026.8+) the
first time any LLM API collects tools — this module is never imported at
Chzzk setup time. It answers only for Chzzk's own API id (see `.llm_api`),
...
def async_get_tools(
    ...
    """Return Chzzk's tools for the Chzzk API only."""
    if api_id != LLM_API_ID:
        return None
    return LLMTools(tools=_TOOLS, prompt=API_PROMPT)
